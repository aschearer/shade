...
public class GroundLight extends Body implements LightSource, ShadowEntity {
    ...
    public void renderLight(Graphics g, int width, int height) {
        ...
        /*
         * ,new GradientFill( new Vector2f(0,0),new Color(0,0,0,intensity), new
         * Vector2f(myCastLength,0), new Color(0,0,0,0) )
         */
        );
    ...
    public void onCollision(Entity obstacle) {
        ...
        if (obstacle.getRole() == Role.PLAYER) {
            ...
            if (minx < miny) {
                ...
                if (Math.abs(playerx - getCenterX() - velx) > Math.abs(playerx
                        - getCenterX()))
                    velx = -velx;
                move(-velx, 0);
            } else {
                if (Math.abs(playery - getCenterY() - vely) > Math.abs(playery
                        - getCenterY())) {
                    vely = -vely;
                }
                move(0, -vely);
